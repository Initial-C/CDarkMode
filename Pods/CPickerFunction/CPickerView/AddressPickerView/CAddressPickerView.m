...
@interface CAddressPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
// 选中后的回调
@property (nonatomic, copy) CAddressResultBlock resultBlock;

@end

@implementation CAddressPickerView
...
+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr isAutoSelect:(BOOL)isAutoSelect resultBlock:(CAddressResultBlock)resultBlock {
    CAddressPickerView *addressPickerView = [[CAddressPickerView alloc] initWithDefaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect resultBlock:resultBlock];
    [addressPickerView showWithAnimation:YES];
}

#pragma mark - 初始化地址选择器
- (instancetype)initWithDefaultSelected:(NSArray *)defaultSelectedArr isAutoSelect:(BOOL)isAutoSelect resultBlock:(CAddressResultBlock)resultBlock {
    if (self = [super init]) {
        // 默认选中
...
#pragma mark - 获取地址数据
- (void)loadData {
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"CCity" ofType:@"plist"];
    NSArray *arrData = [NSArray arrayWithContentsOfFile:filePath];
    for (NSDictionary *dic in arrData) {
        // 此处用 YYModel 进行解析
        CProvinceModel *proviceModel = [CProvinceModel yy_modelWithDictionary:dic];
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    CProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
    CCityModel *cityModel = provinceModel.city[rowOfCity];
    if (component == 0) {
        //返回省个数
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    if (component == 0) {//省
        CProvinceModel *provinceModel = self.addressModelArr[row];
        showTitleValue = provinceModel.name;
    }
    if (component == 1) {//市
        CProvinceModel *provinceModel = self.addressModelArr[rowOfProvince];
        CCityModel *cityModel = provinceModel.city[row];
    ...
    if (component == 2) {//区
        ...
        if (cityModel.town.count > 0) {
            CTownModel *townModel = cityModel.town[row];
    ...
    if (rowOfProvince < self.addressModelArr.count) {
        ...
        if (rowOfCity < provinceModel.city.count) {
            ...
            if (rowOfTown < cityModel.town.count) {
                CTownModel *townModel = cityModel.town[rowOfTown];
...
- (void)scrollToRow:(NSInteger)firstRow secondRow:(NSInteger)secondRow thirdRow:(NSInteger)thirdRow {
    if (firstRow < self.addressModelArr.count) {
        ...
        CProvinceModel *provinceModel = self.addressModelArr[firstRow];
        if (secondRow < provinceModel.city.count) {
            rowOfCity = secondRow;
            [self.pickerView reloadComponent:1];
            CCityModel *cityModel = provinceModel.city[secondRow];
