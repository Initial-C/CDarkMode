...
+ (NSString *)timeInfoWithStamp:(NSString *)stamp {
...
+ (NSInteger)timestampFromTime:(NSString *)time {
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"yyyy/MM/dd HH:mm:ss"];
    NSDate *date = [formatter dateFromString:time];
    NSTimeInterval doubleTypeTime = [date timeIntervalSince1970];
    return (NSInteger)doubleTypeTime;
}
// special
+ (NSString *)timeInfoWithSpecDate:(NSDate *)date {
    ...
    if (time < 3600) { // 小于一小时
        ...
        return [NSDate stringWithDate:date format:@"yyyy/MM/dd HH:mm"];
    }
}
